...
function normalizePremiumTier(value: unknown) {
  return value === "premiumPlus" ? "premiumPlus" : value === "premium" ? "premium" : null;
}

function createTransferReference(uid: string) {
...
async function deleteStoragePath(storagePath: string | null | undefined) {
  if (!storagePath) {
    return;
  }
...
async function deletePostCascadeAdmin(postId: string) {
  ...
  const parentPostId = typeof postSnapshot.get("parentPostId") === "string" ? postSnapshot.get("parentPostId") as string : null;
  const replyToPostId = typeof postSnapshot.get("replyToPostId") === "string" ? postSnapshot.get("replyToPostId") as string : null;
  const imageStoragePath = typeof postSnapshot.get("imageStoragePath") === "string" ? postSnapshot.get("imageStoragePath") as string : null;
...
async function assertModerator(uid: string) {
...
export const approvePremiumBankTransfer = onCall(async (request) => {
  ...
  let userId = "";
  let approvedTier: "premium" | "premiumPlus" | null = null;
  let premiumUntil = "";
...
export const recordPostView = onCall(async (request) => {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (authorId) transaction.update(db.collection("users").doc(authorId), { totalPostViews: FieldValue.increment(1), updatedAt: FieldValue.serverTimestamp() });
    counted = true;
  });
  return { counted };
});
