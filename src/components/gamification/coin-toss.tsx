...
export function CoinToss() {
  ...
  async function flip() {
    ...
    const payout = result === pickedSide ? Math.min(MAX_GAMBLING_WITHDRAWAL, Math.floor(wager * 1.5)) : 0;
    await settleGamblingRound(user.uid, { wager, payout, xpReward: payout ? Math.max(1, Math.floor(payout / 30)) : 0, title: "Coin toss" });
