...
export function WheelSpin() {
  ...
  const wheelBackground = useMemo(() => {
    let start = 0;
    const tones = ["#f472b6", "#60a5fa", "#34d399", "#fbbf24", "#a78bfa"];
    return `conic-gradient(${SEGMENTS.map((segment, index) => { const end = start + segment.chance; const stop = `${tones[index]} ${start}% ${end}%`; start = end; return stop; }).join(", ")})`;
  }, []);
