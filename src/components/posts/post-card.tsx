                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        await Promise.all(moderators.map((moderatorId) => createNotification({
                          type: "report",
                          title: "Reported to Isreal",
                          body: `${currentUserProfile.displayName} Reported a post to Isreal.`,
                          actorId: currentUserProfile.uid,
                          userId: moderatorId,
                          postId: post.id,
                        })));
                        setMenuOpen(false);
                        toast.success("Reported to Isreal");
                      }}
                    >
                      <Flag size={15} />
                      Report to Isreal
      ...
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1 space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-textMuted">
            ...
            <Button type="button" variant="ghost" size="sm" className="gap-2" onClick={() => navigate(`/post/${post.id}`)}>
      ...
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1 space-y-3">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        await Promise.all(moderators.map((moderatorId) => createNotification({
                          type: "report",
                          title: "Post reported",
                          body: `${currentUserProfile.displayName} Reported to Isreal.`,
                          actorId: currentUserProfile.uid,
                          userId: moderatorId,
                          postId: post.id,
                        })));
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-textMuted">
            ...
            <Button type="button" variant="ghost" size="sm" disabled={submittingReaction} className="gap-2 text-red-500" onClick={() => void handleRottenTomato()}>
              Rotten tomato
              <span className="rounded-full bg-red-500/10 px-2 py-0.5 text-[10px] font-semibold">5 gems</span>
              <span>{post.rottenTomatoCount}/5</span>
            </Button>
            <Button type="button" variant="ghost" size="sm" className="gap-2" onClick={() => navigate(`/post/${post.id}`)}>
              <MessageCircle size={15} />
              {post.replyCount}
